    ...
    def printCustomerReport(self):
        try:
            ...
            for customer in customers:
                ...
                birth_year = int(ic_number[:2]) # extract birth year from beginning of IC number
                if birth_year > current_year % 100: # det century based on birth year logic
                    birth_year += 1900
                else:
                    birth_year += 2000

                age = current_year - birth_year # to calculate age
                if age < 18:
