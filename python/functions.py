...
# Or do I have to create function place this code
# inside the function and call the function?
# How many places are you going to have this line of code
# more than 1 time it must be converted to a function
...
def buylunch(makan, minum):
    ...
    for food in minum:
        packedfood.append(food)
        if (food == "nescafe"): total = total + 3
        if (food == "air suam"): total = total + 0.50
    # you can return a single value
    # return total
    # you can return multiple values
    return [packedfood,total] #bagi resit kat sir
    # code after (return, break, continue) keyword never get executed

# context => Caller context
result = buylunch(["nasi","sayur"],["nescafe","air suam"])
result = result[1] + .5 
# we create result and return the function so that 
# we can do some adjustment or any othe thing like 
# adding the sst in the bill
print("Amount to be Paid: ", result)

# can i have more than one return statement inside the function
# yes you can have more than one return statement
# however once the return statement is executed 
# it will come out of the function
def evenOrOdd(givenNumber):
    if (givenNumber % 2 == 0):
        return "Even Number"
...
# number of arguments is keep changing
# 'Variable Length Arguments'
# Traditionally how we handle such problem is using list
...
# In python to handle variable length argument problem they introduce something
# called *parameter
# When ptyhon sees * it will convert all the arguments to a tuple and pass the 
# tuple to the function
#again
def calculateTotal(numbers):
    total = 0
